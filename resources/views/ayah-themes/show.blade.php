    ...
    <section class="space-y-8">
        <div class="rounded-[2rem] border border-[#d7c8ae] bg-[#fff9ef] px-6 py-6 shadow-[0_20px_44px_rgba(88,67,29,0.08),inset_0_1px_0_rgba(255,255,255,0.82)]">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    <p class="text-sm font-semibold uppercase tracking-[0.28em]" style="color: {{ $theme->badge_color ?: '#0e7c66' }}">Theme</p>
                    @if ($theme->display_title_urdu)
                        <h1 class="mt-2 text-right text-3xl font-semibold text-stone-900 sm:text-4xl" dir="rtl" lang="ur">{{ $theme->display_title_urdu }}</h1>
                    @endif
        ...
        @else
            <div class="rounded-[1.6rem] border border-[#f0d28a] bg-[#fff3d8] px-5 py-4 text-sm text-[#8e5f00] shadow-[0_10px_24px_rgba(166,116,0,0.08)]">
                <p class="font-medium">No ayahs tagged yet for this theme.</p>
                <p class="mt-1" dir="rtl" lang="ur">اس موضوع کے لیے ابھی آیات ٹیگ نہیں کی گئیں۔</p>
            </div>
        @endif
    </section>
</x-layouts.app>
